...
* Collation defines comparison rules such as case sensitivity, accent sensitivity, sort order, and locale; `='A'` vs `='a'` can behave differently across systems
...
* `ONLY_FULL_GROUP_BY` in MySQL is a common SQL mode that rejects grouped queries where non-aggregated columns are not functionally dependent; it helps enforce SQL correctness
...
* MySQL `SQL_MODE` controls strictness and compatibility, including `ONLY_FULL_GROUP_BY`, zero-date handling, and ANSI-like behavior; many production issues come from unexpected mode differences
...
* Indexes: B-tree, unique index, composite index, covering index, partial index (dialect-specific)
* Query plans and execution plans: look for table scans, index scans, sort, hash join, nested loops, and scan-heavy operations
* Views vs materialized views: view is a logical query, materialized view stores precomputed results and needs refresh logic
...
* Semi-join: use `WHERE EXISTS (SELECT 1 FROM ... WHERE ...)` when you only need to know a matching row exists; anti-join uses `NOT EXISTS` for rows without matches
...
* Common join algorithms: nested-loop joins are good for small inner loops and indexed probes; hash joins are efficient for large unsorted data; merge joins work well when both inputs are sorted and can be streamed together
...
* Partition by and order by inside window functions: `ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY order_date DESC)`
* Running totals and cumulative sums with `SUM(amount) OVER (PARTITION BY customer_id ORDER BY order_date)`
...
* Correlated subquery vs nested subquery; use `EXISTS` when checking existence is enough
* Data warehouse performance principles: filter early, project only needed columns, reduce shuffles, avoid unnecessary joins, and use partitioning/clustering
...
* Predicate pushdown and index seeks are usually better than evaluating functions on every row; avoid `WHERE LOWER(email) = 'abc'` when a case-insensitive index or normalized column is available
* Join type matters: nested-loop joins are efficient for selective indexed lookups, hash joins excel on large unsorted sets, and merge joins are ideal when inputs are already sorted on the join key
