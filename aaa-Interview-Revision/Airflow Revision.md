...
```mermaid
flowchart LR
	Author[DAG Author] --> Bundle[DAG Bundle<br/>Local / Git / Object Storage]
	Bundle --> Processor[DAG Processor<br/>DagFileProcessorManager + Parser Processes]
	Processor -->|Serialized DAGs and versions| DB[(Metadata Database)]

	User[Operations User] --> API[Airflow API Server<br/>UI + Stable REST API v2 + Task Execution API]
	API --> DB

	DB --> Scheduler[Scheduler<br/>DAG Runs + Dependencies + Pools + Concurrency]
	Scheduler --> Executor[Executor<br/>Runs inside Scheduler Process]
	Executor --> Local[Local Task Subprocesses]
	Executor --> Queue[Broker / Queue]
	Queue --> Workers[Remote Workers]
	Executor --> Pods[Task Pods / Batch Jobs]

	Workers --> WorkerWrapper[Worker Task Wrapper]
	Pods --> WorkerWrapper
	WorkerWrapper --> TaskProcess[One Subprocess per Task Instance]
	TaskProcess -->|Task SDK / Execution API| API
	TaskProcess --> Logs[(Remote Task Logs)]

	Deferred[Deferred Task] --> Triggerer[Triggerer<br/>Asyncio Event Loop]
	Triggerer -->|Trigger Event| Scheduler
	Scheduler -->|Resume Task| Executor

	HA[Multiple Schedulers<br/>Database Row-Level Locking] -.-> Scheduler
	HA2[Multiple Triggerers<br/>Capacity-Based HA] -.-> Triggerer
```
...
* Required components in Airflow 3:
	...
	* API server command: start the Airflow 3 web/API boundary with `airflow api-server`
	* DAG processor command: start the required standalone parser with `airflow dag-processor`, including local development setups
* Optional components:
...
* Airflow 3 execution boundary: the worker wrapper communicates with the API server through the Task SDK/Execution API; task code should not import metadata ORM sessions
* Scheduler loop: creates eligible DAG runs, evaluates serialized DAG dependencies and task states, applies pools/concurrency/priority, and queues workloads through the executor
* Scheduler high availability: multiple schedulers can run for throughput and resilience, coordinating ownership through metadata-database row-level locking
* DAG processor internals: `DagFileProcessorManager` watches the bundle and starts per-file parser processes, which repeatedly import, validate, serialize, and report DAGs
* DAG processor tuning: parsing throughput is influenced by `parsing_processes`, `min_file_process_interval`, import timeouts, and the number and size of DAG files
* DAG version pinning: a versioned bundle can let a DAG run and its workers use the same code version; local, S3, and GCS bundles may not provide the same version guarantees
...
* Suppose a daily pipeline loads sales data:
	...
	* Worker wrapper starts and supervises one task subprocess for the task instance
	* Task subprocess uses the Task SDK/Execution API through the worker boundary for state, XCom, context, and Airflow resource access
	* API server shows the DAG graph, task states, logs, and retries in the UI

### Airflow 3 Task Execution Architecture

```mermaid
sequenceDiagram
	participant S as Scheduler
	participant E as Executor
	participant W as Worker or Pod
	participant X as Worker Wrapper
	participant T as Task Subprocess
	participant A as API Server
	participant D as Metadata DB

	S->>E: Submit task workload
	E->>W: Queue or launch task
	W->>X: Start task wrapper
	X->>T: Start isolated subprocess
	T->>X: Task SDK / execution requests
	X->>A: State, heartbeat, XCom, context, resource calls
	A->>D: Read or write Airflow metadata
	A-->>X: API response
	X-->>S: Task result through executor
	S->>D: Persist final task state
```
...
* TaskGroup: logical grouping that organizes tasks visually in the UI and keeps a DAG easier to read
* Retry policy: declarative Airflow 3 rule set that decides whether a failed task should retry, fail fast, or use the default retry logic
* Deadline alert: Airflow 3 replacement for SLA-style monitoring for late DAG runs
* Resumable task: a deferred task that is paused and later resumed by the triggerer after an external event is received
* Zombie task: a task instance left in `running` state without a live heartbeat; Airflow marks it failed or retries it after a timeout
...
* Common task states: `none`, `scheduled`, `queued`, `running`, `success`, `failed`, `upstream_failed`, `skipped`, `up_for_retry`, `deferred`, `removed`, `up_for_reschedule`, `restarting`, and `awaiting_input`
* `deferred` means the task has been paused and resumed later by a trigger; it frees worker slots while waiting on an external event
* `up_for_reschedule` is common for sensors in `reschedule` mode; the sensor waits, then wakes up and checks again
* `restarting` means a task was asked to restart while running; this is usually a control-plane operation rather than a normal error path
* `awaiting_input` is used by human-in-the-loop workflows; the task waits for an operator response instead of continuing automatically
...
* Sensor-specific timeout behavior: `execution_timeout` limits each poll attempt, while the sensor `timeout` limits the total time waiting for success
...
* Airflow 3 retry policies: `RetryRule`, `RetryAction`, and `ExceptionRetryPolicy` allow per-exception retry logic such as "retry 429s for 5 minutes, fail immediately on auth errors"
* A retry policy is evaluated in the task worker process and can override the default retry loop without changing task code
* `AirflowFailException` always wins; it bypasses the normal retry policy and marks the task failed immediately
* Airflow 2 SLA semantics were removed in 3.0; use `DeadlineAlert` for late-run monitoring and notifications instead
* Task instance heartbeat timeout: if a task does not heartbeat, Airflow treats it as a zombie, marks it failed, or retries it if retries remain
* A heartbeat timeout often happens after OOM, worker restart, node eviction, or a liveness-probe-driven environment restart
...
## DAG Lifecycle, Scheduling, and Recovery
...
* A DAG can be paused and unpaused from the UI or API; paused DAGs are not scheduled but can still be triggered manually
* A deactivated DAG is one that the scheduler no longer sees in the DAG bundle; it keeps historical metadata but is effectively not active
* Auto-pausing can disable a DAG after repeated failures, which helps prevent a bad workflow from repeatedly running and poisoning downstream systems
* Clearing a task instance reruns it by resetting its state and incrementing retry metadata; use it only after fixing the root cause
* Backfill runs historical intervals; catchup creates missed scheduled runs automatically after a DAG is enabled or re-enabled
* When a DAG is deleted from the metadata database, historical runs are cleared, but the file in the DAG bundle can make it reappear on the next parse
* The operational sequence is usually: pause or fix the DAG, clear failed task instances or backfill the correct interval, then validate the recovery before re-enabling scheduling
...
* Deferral is available to class-based operators; ordinary `PythonOperator` and TaskFlow Python functions cannot call deferral directly.
* A deferred operator resumes as a new operator instance, so local `self` attributes and local variables are not durable state.
* Triggerers support high availability; multiple triggerers share deferred work using database coordination and capacity limits.
* Human-in-the-loop tasks use the scheduler-managed `awaiting_input` state and do not require the triggerer.
* Airflow 3.2 async tasks are different from deferrable operators: async tasks keep the task process alive and multiplex I/O, while deferral releases the worker slot.
...
* Executor placement: configure multiple executors globally and select a configured executor at DAG or task level; the executor name is stored with the task instance.
* Removed hybrid executors: `LocalKubernetesExecutor` and `CeleryKubernetesExecutor` are not supported in Airflow 3; use multiple executor configuration instead.
* Worker execution model: a remote worker does not run task code in its own long-lived process; it starts, supervises, and tears down a separate subprocess for each task instance.
* LocalExecutor execution model: local task subprocesses run under the scheduler process, so scheduler memory and CPU sizing must include concurrent task processes.
...
* Airflow 3 startup: run `airflow api-server`, `airflow dag-processor`, and `airflow scheduler`; add workers, a broker, and triggerers according to the executor and workload.
* Stable REST API: Airflow 3 replaces `/api/v1` with the FastAPI-based `/api/v2` interface.
* Standard provider: install `apache-airflow-providers-standard` for common operators and sensors such as `BashOperator`, `PythonOperator`, and `ExternalTaskSensor`.
* Authentication: Simple Auth is the default auth manager; FAB compatibility requires installing the FAB provider and configuring `FabAuthManager`.
* Direct metadata access migration: use the Task SDK or Airflow Python Client instead of task code importing metadata database sessions or ORM models.
